...
# Faker library is only being used to assign random names to faces identified
# I added it to prevent the UI window popup which reads name input. This must be removed
...
# Sends the cropped faces to a NodeJS server
def send_img(frame):
...
class FrameProcessor:
    ...
    def process(self, frame):
        ...
        # rois is the number of faces detected in the current frame
        if self.QUEUE_SIZE < len(rois):
            log.warning('Too many faces for processing. Will be processed only {} of {}'
                        .format(self.QUEUE_SIZE, len(rois)))
            rois = rois[:self.QUEUE_SIZE] #only QUEUE_SIZE number of faces will get processed, others will be just ignored

        landmarks = self.landmarks_detector.infer((frame, rois))
        face_identities, unknowns = self.face_identifier.infer((frame, rois, landmarks))
        # face identities =  id of face detected
...
def main():
    ...
    cap = open_images_capture(args.input, False) # Boolean parameter to Enable reading the input in a loop.
